...
std::vector<float> AnnexML::computeScores(const std::vector<std::pair<int, float>>& input) const {
  std::vector<std::vector<std::pair<int, float>>> data_vec = {input};
  DataPartitioner().NormalizeData(&data_vec);
  std::vector<std::pair<int, float>> norm_input = data_vec[0];

  size_t num_used_learner = param_.num_learner();
  if (num_used_learner > partitioning_vec_.size()) { num_used_learner = partitioning_vec_.size(); }

  std::unordered_map<size_t, int> emb_result;
  for (size_t l = 0; l < num_used_learner; ++l) {
    size_t cluster = partitioning_vec_[l].GetNearestCluster(norm_input);
    std::vector<std::pair<size_t, float>> idx_vec;
    embedding_vec_[l].Search(norm_input, cluster, param_.num_nn(), param_.search_eps(), &idx_vec);
    for (const auto& idx : idx_vec) {
      emb_result[idx.first] += 1;
    }
  }

  std::unordered_map<int, float> l_map;
  for (const auto& [idx, count] : emb_result) {
    for (int label : labels_[idx]) {
      l_map[label] += count;
    }
  }

  int max_lid = 0;
  for (const auto& label_vec : labels_) {
    for (int l : label_vec) {
      if (l > max_lid) max_lid = l;
    }
  }
  std::vector<float> scores(max_lid + 1, 0.0);
  for (const auto& [label, score] : l_map) {
    if (static_cast<size_t>(label) < scores.size()) scores[label] = score; // Cast label to size_t
  }
